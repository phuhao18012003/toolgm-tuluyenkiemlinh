...
{do}[{trang}{kt_code}{do}] \033[1;35mADMIN:\033[1;     CHÍ MUM
{do}[{trang}{kt_code}{do}] \033[1;31mZALO: \033[1;33mhttps://zalo.me/0834617174
\033[1;37m────────────────────────────────────────────────────────────"""
        ...
        try:
            ...
            try:
                if response.json()['status_code'] == 0:
                    _lock.acquire()
                    print(f'{do}[{vang}SUBRE62.COM{do}] {do}[{xanh_la}BUFF VIEW SUCCESS{do}] {do}{xanh_duong}+: {trang}{success}{do} View')
                    success += 1
                    _lock.release()
            except:
...
if __name__ == "__main__":
    ...
    print(banner)
    try:
        link = input(f'{do}[{trang}{kt_code}{do}] {xanh_la} NHẬP LINK VIDEO: {trang}')
        print(f'{trang}- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -')
        __aweme_id = str(
